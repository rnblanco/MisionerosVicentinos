<?php
	...
	class EditarSeminario extends DB{
		...
		public function Cargar(){
			...
			$buscarPublicacion = $conexion->prepare("SELECT * FROM seminarios WHERE id_seminario = ? ");
			$buscarPublicacion->execute([$id]);
			$Publicacion = $buscarPublicacion->fetchAll();

			foreach($Publicacion as list($idHistorial, $pais, $texto, $imagen)){
				...
				echo"
                    ...
                    <div class='form-group'> <label class='form-control-label'>Imagen * :</label> <br><img src='../../../../backend/images/seminarios-images/$imagen' id='preview' class='img-thumbnail' style='height:250px!important;width:300px!important;'><br><div id='msg'></div><input type='file' name='img' id='img' class='file' accept='image/*' onblur='validate1(3)'><div class='input-group my-3'><input type='text' class='form-control' disabled placeholder='$imagen2[2]' id='file'><div class='input-group-append'><button type='button' class='browse btn btn-primary'>Subir</button></div></div></div> 
                    <div class='form-group'> <label class='form-control-label'>Título:</label> <input onkeypress='javascript:return tprotection(event)'' type='text' id='pais' class='form-control' onblur='validate1(1)' value='$pais'></div>
	...
	else require_once 'editar-seminario.view.php';
?>